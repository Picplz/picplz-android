...
class UserTypeInfoHandler {
    fun process(
        intent: SignUpCommonIntent,
        state: SignUpCommonState,
    ): SignUpCommonState? {
        return when (intent) {
            is ClickUserTypeButton -> {
                val newUserType =
                    if (state.selectedUserType == intent.userType) {
                        null
                    } else {
                        intent.userType
                    }
                val newPhotographerSelectionState =
                    if (newUserType == UserType.Photographer) {
                ...
                val newUserSelectionState =
                    if (newUserType == UserType.User) {
                        SelectionState.SELECTED
