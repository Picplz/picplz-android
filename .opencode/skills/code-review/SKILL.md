...
```
spawn_sub_agent(
  subagent_type="pi:code-reviewer",
  model="opus",
...
### Tool Usage
Before first MCP tool use, call `# (pi: tools auto-discovered)` to discover deferred MCP tools.
Use `mcp__x__corresponding pi subagent.
Use pi subagent tool directly.
